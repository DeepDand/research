...
print("Training set now has {0[0]} rows and {0[1]} columns".format(X.shape))
print("Test set now has {0[0]} rows and {0[1]} columns".format(X_test.shape))
...
print("\nSupport vector classifier on kpca")
#grid search for best set of hyper parameters
parameters = {'kernel':['rbf'], 'C':np.logspace(-5,15,21,base=2),
'gamma':np.logspace(-15,3,19,base=2), 'epsilon':np.linspace(0,0.5,11)}
svr = SVR()
...
print(best)
#past best were: 
#best = {'epsilon': 0.0, 'C': 32.0, 'gamma': 0.5, 'kernel': 'rbf'}
...
#svr
svr = SVR(kernel='rbf', epsilon=best['epsilon'], C=best['C'], gamma=best['gamma'], max_iter=-1, verbose=True)
svr.fit(X_train, y_train)
pred = np.around(svr.predict(X_train)).astype(int)
pred[pred<0]=0
pred[pred>9]=9
...
df.index+=1
df.columns=['Label']
df.to_csv('result-svm-kpca.csv', header=True)
